...
async def stream_openai_answer(messages_list, user_query) -> AsyncGenerator:
    ...
    usermsgs = " | ".join([a["content"][:100] for a in messages_list if a["role"] == "user"][-3:])
    assistanmsgs = " | ".join([a["content"][:100] for a in messages_list if a["role"] == "assistant"][-3:])
...
async def simple_search_chat_agent(
    ...
    """
    Main chat agent function with incremental web search
    ...
    Yields:
        JSON strings with reasoning steps and content
    """
    ...
    # Check if this is a repeat query from recent history
    ...
    # yield json.dumps({
    #     "type": "reasoning",
    #     "step": "Starting Response",
    #     "content": "Analyzing query requirements",
    #     "query": user_query,
    #     "category": "Overview",
    #     "timestamp": datetime.now(timezone.utc).isoformat()
    # }) + "\n"
    
    # STEP 1: Use query transformer FIRST to decide if web search is needed
    print(f"[DEBUG] ===== STEP 1: Query Analysis =====")
    
    # Extract only user queries from conversation history for the transformer
    past_user_queries = [msg["content"] for msg in conversation_history if msg["role"] == "user"]
    
    print(f"[DEBUG] Past user queries: {past_user_queries}")
    ...
    print(f"[DEBUG] Query analysis complete: needs_web={needs_web}, search_query='{search_query}'")
    ...
    if not needs_web:
        if not route_printed:
            print(f"[DEBUG] ===== ROUTE: DIRECT ANSWER (Transformer Decision) =====")
            route_printed = True
        
        # yield json.dumps({
        #     "type": "reasoning",
        #     "step": "Direct Answer Mode",
        #     "content": f"No web search needed.",
        #     "query": user_query,
        #     "category": "Overview",
        #     "timestamp": datetime.now(timezone.utc).isoformat()
        # }) + "\n"
        ...
        async for stream_chunk in stream_openai_answer(messages_list, user_query):
            if stream_chunk.get('needs_web'):
                ...
                # yield json.dumps({
                #     "type": "reasoning",
                #     "step": "Web Search Override",
                #     "content": "LLM requested web search despite initial analysis",
                #     "needs_web": True,
                #     "query": user_query,
                #     "category": "Override",
                #     "timestamp": datetime.now(timezone.utc).isoformat()
                ...
                # Re-run transformer with current query to get search query
                print(f"[DEBUG] Re-running transformer after override...")
    ...
    # If transformer said web search IS needed, skip direct answer and go to search
        ...
        # yield json.dumps({
        #     "type": "reasoning",
        #     "step": "Web Search Required",
        #     "content": f"Transformer determined web search is needed. Search query: {search_query}",
        #     "needs_web": True,
        #     "query": user_query,
        #     "category": "Overview",
        #     "timestamp": datetime.now(timezone.utc).isoformat()
    ...
    #     "type": "reasoning",
    #     "step": "Searching Web",
    #     "content": f"Performing Google search: {search_query}",
    #     "query": search_query,
    #     "category": "Web Search",
    #     "timestamp": datetime.now(timezone.utc).isoformat()
    # }) + "\n"
    ...
    try:
        print("CALLING GOOGLE ->"  + search_query)
    ...
    if not all_search_results or len(all_search_results) == 0:
        ...
        #     "content": "Google search returned no results",
        #     "query": search_query,
        #     "category": "Web Search",
        #     "timestamp": datetime.now(timezone.utc).isoformat()
        # }) + "\n"
    ...
    #     "content": f"Found {len(all_search_results)} search results",
    #     "query": search_query,
    #     "category": "Web Search",
    #     "timestamp": datetime.now(timezone.utc).isoformat()
    # }) + "\n"
    
    # Step 3: Try answering with all snippets first
    # yield json.dumps({
    #     "type": "reasoning",
    #     "step": "Processing Snippets",
    #     "content": f"Analyzing all {len(all_search_results)} search snippets",
    #     "query": search_query,
    #     "category": "Web Search",
    #     "timestamp": datetime.now(timezone.utc).isoformat()
    # }) + "\n"
    
    print(f"[DEBUG] ===== STEP: Trying to answer with snippets =====")
    
    # yield json.dumps({
    #     "type": "reasoning",
    #     "step": "Generating Answer",
    #     "content": f"Creating response from {len(all_search_results)} snippets",
    #     "query": search_query,
    ...
    async for stream_chunk in answer_from_snippets_streaming(search_query, all_search_results, conversation_history, is_scraped=False):
        if stream_chunk.get('needs_more'):
            ...
            print(f"[DEBUG] ===== DECISION: LLM requested more sources, initiating scraping =====")
            
            # yield json.dumps({
            #     "type": "reasoning",
            #     "step": "Needs Detailed Content",
            #     "content": "Snippets insufficient, scraping top 3 URLs for full content",
            #     "query": search_query,
            #     "category": "Web Search",
            #     "timestamp": datetime.now(timezone.utc).isoformat()
            # }) + "\n"
            ...
            if stream_chunk.get('done'):
                print(f"[DEBUG] ===== DECISION: Snippets were sufficient, answer complete =====")
                return
    ...
    if needs_scraping:
        ...
        #     "type": "reasoning",
        #     "step": "Scraping URLs",
        #     "content": f"Fetching detailed content from {len(top_3_urls)} pages (this may take up to 2 minutes)...",
        #     "query": search_query,
        #     "category": "Web Search",
        #     "timestamp": datetime.now(timezone.utc).isoformat()
        # }) + "\n"
        ...
        if not scraped_data or not scraped_data.get('results'):
            print(f"[DEBUG] ===== SCRAPING: Failed to fetch content =====")
            
            # yield json.dumps({
            #     "type": "reasoning",
            #     "step": "Scraping Failed",
            #     "content": "Could not fetch detailed content from pages",
            #     "query": search_query,
            #     "category": "Error",
            #     "timestamp": datetime.now(timezone.utc).isoformat()
            # }) + "\n"
